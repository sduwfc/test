...
class SGRunner(object):
    ...
    def __init__(self, args):
        ...
        self.task_map = {}
        self.model_type = getattr(args, "model_type", "pt")
    ...
    def wait_result(self):
        ...
        while not (self.query_count <= 0):
            ...
            if self.model_type == "tf":
                ...
                start_scores = (
                    start_scores.astype(np.float32) * self.out_info[self.key1]["scale"]
                )
                end_scores = (
                    end_scores.astype(np.float32) * self.out_info[self.key2]["scale"]
                )
            res = []
    ...
    def run_one_query(self, query_samples):
        ...
        if len(query_samples) % self.batch_size != 0:
            padding_sample_num = self.batch_size - (
                len(query_samples) % self.batch_size
            )
        ...
        print(f"Use order of type {self.model_type}")
        ...
        for idx in bar:
            ...
            if self.model_type == "tf":
                task_id = self.runner.put(
                    input_ids_data, segment_ids_data, input_mask_data
                )
            elif self.model_type == "pt":
                task_id = self.runner.put(
                    input_mask_data, input_ids_data, segment_ids_data
                )
            ...
            bar.set_description(
                "Put task_id {:d} with shape = {:}".format(
                    task_id, input_ids_data.shape
                ),
                True,
            )
